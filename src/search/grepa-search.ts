...
export class GrepaSearch {
  ...
  private static shouldIncludeFile(file: string, options: SearchOptions): boolean {
    if (options.files && options.files.length > 0) {
      return options.files.some(pattern => file.includes(pattern));
    }

    if (options.exclude && options.exclude.length > 0) {
      return !options.exclude.some(pattern => file.includes(pattern));
    }
  ...
  private static matchesSearch(anchor: MagicAnchor, options: SearchOptions): boolean {
    ...
    return options.markers.some(marker => 
      anchor.markers.some(anchorMarker => 
