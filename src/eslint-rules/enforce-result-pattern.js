// :A: tldr Custom ESLint rule to enforce Result pattern usage
module.exports = {
  meta: {
    type: 'problem',
    docs: {
  ...
  create(context) {
    // :A: ctx track Result-returning functions
    const resultReturningFunctions = new Set();

    return {
      // :A: api detect functions that return Result type
      ...
      // :A: api detect arrow functions that return Result
      ...
      // :A: api check for throw statements in Result-returning functions
      ThrowStatement(node) {
      ...
      // :A: api check for try-catch in async functions
      TryStatement(node) {
        const func = context.getAncestors().reverse().find(
      ...
      // :A: api check for direct toast.error calls
      CallExpression(node) {
        if (
        ...
        // :A: ctx check for unchecked Result objects
        if (node.callee.type === 'Identifier') {
          ...
          if (
            ...
            // :A: ctx look for subsequent if (!result.ok) check
